<div class="px-5 sm:px-20">
    @php
        $steps = [['number' => 1, 'text' => 'Invoice Sent', 'status' => $interpretation->invoiceSent == 1 || ($interpretation->user->role_id == 1 || $interpretation->user->role_id == 2) ? 'success' : ''], ['number' => 2, 'text' => 'Payment Paid', 'status' => $interpretation->paymentStatus == 2 || $interpretation->paymentStatus == 1 ? 'success' : ''], ['number' => 3, 'text' => 'Interpreter Assigned', 'status' => $interpretation->invoiceSent == 1 && ($interpretation->paymentStatus == 2 || $interpretation->paymentStatus == 1) && $interpretation->interpreter_id != null ? 'success' : ''], ['number' => 4, 'text' => 'Interpretation Completed', 'status' => $interpretation->interpreter_completed == 1 ? 'success' : '']];
    @endphp
    @php
        $prevStatus = '';
