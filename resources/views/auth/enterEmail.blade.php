                ...
                <div class="h-screen xl:h-auto flex py-5 xl:py-0 my-10 xl:my-0">
                    <div class="my-auto mx-auto xl:ml-20 bg-white dark:bg-darkmode-600 xl:bg-transparent px-5 sm:px-8 py-8 xl:p-0 rounded-md shadow-md xl:shadow-none w-full sm:w-3/4 lg:w-2/4 xl:w-auto">
                        @if(session('emailNotFound'))
                            <div class="alert alert-danger-soft show flex items-center mb-2 mt-2" role="alert">{{ session('emailNotFound') }}
                            </div>
                        @endif                    
                        @if(session('emailNotVerified'))
                            <div class="alert alert-danger-soft show flex items-center mb-2 mt-2" role="alert">{{ session('emailNotVerified') }}
                            </div>
                        @endif
                        ...
                        @endif 
                            ...
                            <div class="intro-x mt-8">
                                <input type="email" name="email" class="intro-x login__input form-control py-3 px-4 block" placeholder="Enter Email Address">
                            </div>
                            <div class="intro-x mt-5 xl:mt-8 text-center xl:text-left">
                                <button style="width: 200px;" type="submit" class="btn btn-primary py-3 px-4 w-full xl:w-32 xl:mr-3 align-top">Send Reset Link</button>
                        @if(isset($isContractor) && $isContractor == 1)
                            <a href="{{ route('contractor.login') }}" class="btn btn-outline-secondary py-3 px-4 w-full xl:w-32 mt-3 xl:mt-0 align-top">Go Back</a>
                                {{-- <a href="{{ route('admin.login') }}" class="btn btn-outline-secondary " style="margin: auto; margin-left: 10px;"><i data-lucide="unlock"></i></a> --}}
                        @else
                            <a href="{{ route('login') }}" class="btn btn-outline-secondary py-3 px-4 w-full xl:w-32 mt-3 xl:mt-0 align-top">Go
                                    Back</a>
                        @endif
                            </div>
                        </form>
