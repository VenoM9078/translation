                        ...
                        <table id="myTable" class="table table-striped" style="width:100%">
                            ...
                            <tbody>
                                @foreach ($proofReadRequests as $proofReadRequest)
                                    <tr>
                                        <td class="whitespace-nowrap">{{ $proofReadRequest->order->user->name }}</td>
                                        <td class="whitespace-nowrap">{{ $proofReadRequest->order->user->email }}</td>

                                        @if ($proofReadRequest->order->user->casemanager == '' && $proofReadRequest->order->user->access_code == '')
                                            <td class="whitespace-nowrap">N/A</td>
                                            <td class="whitespace-nowrap">N/A</td>
                                        @else
                                            <td class="whitespace-nowrap">{{ $proofReadRequest->order->user->casemanager }}
                                            </td>
                                            <td class="whitespace-nowrap">{{ $proofReadRequest->order->user->access_code }}
                                            </td>
                                        @endif
                                        <td class="whitespace-nowrap">{{ $proofReadRequest->order->worknumber }}</td>
                                        <td class="whitespace-nowrap">{{ $proofReadRequest->order->user->email }}</td>
                                        <td class="whitespace-nowrap">
                                            {{ $proofReadRequest->order->translationRequest->translator_email }}</td>
                                        ...
                                        @endif
                                        <td class="whitespace-nowrap">
                                            {{ App\Helpers\HelperClass::convertDateToCurrentTimeZone($proofReadRequest->created_at, request()->ip()) }}
                                        </td>
                                        ...
                                        <td class="whitespace-nowrap">
                                            ...
                                            class="btn btn-warning mr-1 mb-2"> <i data-lucide="download"
                                                ...
                                                @if ($proofReadRequest->proofread_status == 0)
                                                    <a href="{{ route('changeProofReadRequestStatus', $proofReadRequest->order->id) }}"
                                                        class="btn btn-warning mr-1 mb-2"> Mark as Completed </a>
                                                    {{-- <a href="{{ route('mailToTranslator',$translationRequest->order->id) }}"
                                            class="btn btn-primary mr-1 mb-2"> Send Mail Again </a> --}}
                                                @else
                                                    <a href="{{ route('changeProofReadRequestStatus', $proofReadRequest->order->id) }}"
                                                        class="btn btn-danger mr-1 mb-2"> Mark as Incomplete </a>
                                                    {{-- <a href="{{ route('mailToProofReader',$translationRequest->order->id) }}"
                                            class="btn btn-dark mr-1 mb-2"><i data-lucide="calendar"
