                    ...
                    <tr>
                        ...
                        <td class="whitespace-nowrap">
                            <div class="flex  items-center">
                                <a href="javascript:;" data-trigger="click" title="{{ $order->orderStatus }}"
                                    class="tooltip btn btn-primary mr-1 mb-2">Show Progress</a>

                                <a href="{{ route('downloadFiles', $order->id) }}" class="btn btn-warning mr-1 mb-2"> <i
                                        data-lucide="download" class="w-5 h-5"></i> </a>

                                <a href="{{ route('mailToTranslator', $order->id) }}" class="btn btn-success mr-1 mb-2"> <i
                                        data-lucide="calendar" class="w-5 h-5 mr-2"></i> Mail to Translator </a>
                                <form action="{{ route('destroy', $order->id) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <div> <button type="submit" class="btn btn-danger mb-2"><i data-lucide="trash"
                                                class="w-4 h-4"></i></button> </div> <!-- END: Modal Toggle -->
                                </form>
